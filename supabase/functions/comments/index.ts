...
async function handleSubmit(req: Request): Promise<Response> {
  ...
  const { location_id, blog_slug, blog_title, author_name, author_email, comment_text, honeypot } = body;
  ...
  if (!location_id || !blog_slug || !author_name || !author_email || !comment_text) {
    return new Response(
  ...
  const sanitizedComment = {
    location_id: sanitizeText(location_id),
...
async function handleFetch(req: Request): Promise<Response> {
  ...
  if (!locationId || !blogSlug) {
    return new Response(
      JSON.stringify({ error: "location_id and blog_slug are required" }),
      { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  }

  // Fetch approved comments for this blog post at this location
